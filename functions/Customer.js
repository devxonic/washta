...
const updateRefreshToken = async (req, token) => {
  let player = await CustomerModel.findOneAndUpdate(
...
const editProfile = async (req) => {
  ...
  let car = await VehiclesModel.findOneAndUpdate(
    { _id: req.body.car._id },
    { $set: { ...req.body.car } },
    {
      new: true,
    }
...
const getProfile = async (req) => {
  ...
  let car = await VehiclesModel.findOne(
    { $and: [{ Owner: Customer._id }, { isSelected: true }] },
    { __v: 0 }
  );
  return { Customer, car };
};
...
const updateImage = async (req, resizedAvatar, originalAvatar) => {
  let customer = await CustomerModel.findByIdAndUpdate(
    ...
    {
      $set: {
        avatar: originalAvatar.Location,
        resizedAvatar: resizedAvatar.Location,
      },
    },
    {
      new: true,
...
const updateNotification = async (req) => {
  let player = await CustomerModel.findByIdAndUpdate(
    { _id: req.user.id, isTerminated: { $ne: true } },
...
const getNotification = async (req) => {
  let player = await CustomerModel.findOne(
    { _id: req.user.id, isTerminated: { $ne: true } },
    { password: 0, __v: 0 }
  );
...
const logout = async (req) => {
  ...
  if (req.user.role == "seller") {
    let User = await SellerModel.findByIdAndUpdate(
      { _id: req.user.id },
      { $set: { sessionKey: "" } }
    );
    return User;
...
const addVehicles = async (req) => {
  let { vehicleManufacturer, vehiclePlateNumber, vehicleName, vehicleType } =
    req.body;
...
const updateVehicles = async (req) => {
  let { id } = req.params;
  let vehicle = await VehiclesModel.findOneAndUpdate(
    { $and: [{ Owner: req.user.id }, { _id: id }] },
    { ...req.body },
    { new: true }
  );
  return vehicle;
...
const deleteVehicle = async (req) => {
  let { id } = req.params;
  let vehicle = await VehiclesModel.findByIdAndDelete({
    _id: id,
    Owner: req.user.id,
  });
  return vehicle;
...
const getIsSelected = async (req) => {
  let Vehicles = await VehiclesModel.findOne({
    $and: [{ Owner: req.user.id }, { isSelected: true }],
  });
  return Vehicles;
};
...
const updateIsSelected = async (req) => {
  let id = req.body.id;
  let Vehicles = await VehiclesModel.findOneAndUpdate(
    { $and: [{ Owner: req.user.id }, { isSelected: true }] },
    { isSelected: false }
  );
  let Selected = await VehiclesModel.findOneAndUpdate(
    { $and: [{ Owner: req.user.id }, { _id: id }] },
    { isSelected: true },
    { new: true }
  );
...
const getAllShops = async (req) => {
  ...
  for (const shop of Shops) {
    let shopReviews = await ReviewModel.find({
      shopId: shop?._id,
      isDeleted: { $ne: true },
    });
    let shopOrders = await OrderModel.find({
      shopId: shop?._id,
      status: "completed",
    }).count();
...
const getShopsServicefee = async (req) => {
  ...
  let ServiceFee = await serviceModel.find({
    applyAt: id,
    ApplicableStatus: { $ne: false },
  });
  return ServiceFee;
};
...
const getShopsPromoCode = async (req) => {
  ...
  let findFilter = {
    isDeleted: { $ne: true },
    isActive: { $ne: false },
    $or: [{ giveTo: id }, { giveToAll: { $ne: false } }],
...
const getMyBookingById = async (req) => {
  let Bookings = await OrderModel.findOne({
...
const cancelBooking = async (req) => {
  ...
  let date = new Date();
  let Bookings = await OrderModel.findByIdAndUpdate(
    req.params.id,
    {
      isCancel: true,
      cancelBy: "customer",
      cancellationResion: cancellationResion,
      status: "cancelled",
...
const createNewBooking = async (req) => {
  ...
  let getShopsFee = await ServiceModel.find({
    $and: [
      { ApplicableStatus: true },
      { $or: [{ applyAtAll: true }, { applyAt: req.body.shopId }] },
    ],
  });
  console.log("shop fee =>", getShopsFee);

  let paymentId = crypto.randomUUID();
  console.log(paymentId);
  let Shop = await shopModel
    .findOne({ _id: req.body.shopId }, { shopName: 1, cost: 1 })
    .populate({
      path: "Owner",
      select: {
        bankAccount: 1,
  ...
  console.log("get Shops ", Shop);
  let amount = parseFloat(Shop.cost);
  let washtaFee = 0;
  if (getShopsFee && getShopsFee?.length > 0) {
  ...
  if (!Shop.cost) return { error: "invaild shop data ! couldn't found" };
  ...
  if (req.body.promoCode) {
    ...
    if (promoCode.Discounttype == "fixed") {
      promoCodeDiscount += parseFloat(promoCode.discount);
      finalCost -= parseFloat(promoCode.discount);
    }
    if (req.body.promoCode.Discounttype == "percentage") {
      promoCodeDiscount += parseFloat((promoCode.discount / 100) * amount);
      finalCost -= parseFloat((promoCode.discount / 100) * amount);
    }
  }
  ...
  req.body.discount = promoCodeDiscount; 
...
const getbookingbyStatus = async (req) => {
  let Bookings = await OrderModel.find({
    $and: [{ customerId: req.user.id }, { status: req.query.status }],
  }).populate([
    {
      path: "customerId",
      select: {
...
const getAllInvoice = async (req) => {
  ...
  let updatedOrder = orders.map((order) => {
    if (order?._doc?.orderCompleteAt && order?._doc?.orderAcceptedAt) {
      return (order._doc = {
        ...order._doc,
        duration: getTimeDifferenceFormatted(
          order._doc.orderAcceptedAt,
          order._doc.orderCompleteAt
        ),
      });
    }
    return order._doc;
...
const getInvoiceById = async (req) => {
  ...
  if (orders?._doc?.orderAcceptedAt && orders?._doc?.orderCompleteAt) {
    orders._doc = {
      ...orders._doc,
      duration: getTimeDifferenceFormatted(
        orders._doc.orderAcceptedAt,
        orders._doc.orderCompleteAt
      ),
    };
  }
  return orders?._doc;
...
const getMyReviews = async (req) => {
  ...
  if (shopId) {
    let Rating = await ReviewModel.find({
      customerId: id,
      shopId,
      isDeleted: { $ne: true },
    }).populate(populate);
  ...
  if (sellerId) {
    let Rating = await ReviewModel.find({ customerId: id, sellerId }).populate(
      populate
    );
    let FormatedRating = formateReviewsRatings?.(Rating);
    return FormatedRating;
...
const updatesShopReview = async (req) => {
  ...
  let Rating = await ReviewModel.findOneAndUpdate(
    { _id: reviewId, customerId: req.user.id, isDeleted: { $ne: true } },
    { rating, "comment.text": comment.text },
    { new: true, fields: { rating: 1, comment: 1 } }
  );
  if (!Rating) return Rating;
  let FormatedRating = formateReviewsRatingsSingle?.(Rating);
  return FormatedRating;
...
const deleteShopReviews = async (req) => {
  ...
  let Review = await ReviewModel.findOneAndUpdate(
    { _id: reviewId, customerId: req.user.id, isDeleted: { $ne: true } },
    { isDeleted: true, deleteBy: { id: req.user.id, role: "customer" } },
    { new: true }
  );
...
const getShopReviews = async (req) => {
  ...
  let Reviews = await ReviewModel.find({ shopId, isDeleted: { $ne: true } })
    .sort({ createdAt: 1 })
    .limit(limit ?? null)
    .populate(populate);
...
const getSellerReview = async (req) => {
  ...
  if (sellerId) {
    let Reviews = await ReviewModel.find({ sellerId, isDeleted: { $ne: true } })
      .sort({ createdAt: 1 })
      .limit(limit ?? null)
      .populate(populate);
  ...
  if (shopId) {
    let owner = await shopModel.findOne(
      { _id: shopId, isTerminated: { $ne: true } },
      { Owner: 1 }
    );
    if (!owner) return null;
    let Reviews = await ReviewModel.find({
      sellerId: owner.Owner,
      isDeleted: { $ne: true },
    })
      .sort({ createdAt: 1 })
      .limit(limit ?? null)
...
const getAgentReview = async (req) => {
  ...
  let Reviews = await ReviewModel.find({ agentId, isDeleted: { $ne: true } })
    .sort({ createdAt: 1 })
    .limit(limit ?? null)
    .populate(populate);
  let FormatedRating = formateReviewsRatings?.(Reviews);
